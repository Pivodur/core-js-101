...
function encodeToRot13(str) {
  ...
  const output = Array.from(str).map((char) => {
    if (uppercase.includes(char)) {
      const cipherIndex = (uppercase.indexOf(char) + 13) % 26;
      return uppercase[cipherIndex];
    }
    if (lowercase.includes(char)) {
      const cipherIndex = (lowercase.indexOf(char) + 13) % 26;
      return lowercase[cipherIndex];
    }
    return char;
  });
